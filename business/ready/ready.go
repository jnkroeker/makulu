...
func checkDB(ctx context.Context, url string) error {
	// the context the user passes in to Validate governs how long the for loop runs
	// I create a new context here with half second timeout so retry is called before
	// pinging the database again
	ctx, cancel := context.WithTimeout(ctx, 500*time.Millisecond)
	defer cancel()

	// if http call to health endpoint ok, then db is ready
